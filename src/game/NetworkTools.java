...
public class NetworkTools {
    ...
    public static double[] createZeroArray(int size, double lower_bound, double upper_bound){
        if(size < 1){
            return null;
        }
        double[] ar = new double[size];
    ...
    public static double[][] createZeroArray(int sizeX, int sizeY, double lower_bound, double upper_bound){
        if(sizeX < 1 || sizeY < 1){
            return null;
        }
        double[][] ar = new double[sizeX][sizeY];
        for(int i = 0; i < sizeX; i++){
            ar[i] = createZeroArray(sizeY, lower_bound, upper_bound);
        }
        return ar;
    }
